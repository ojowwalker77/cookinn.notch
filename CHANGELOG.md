...
### Added
- **HTTP hooks**: Event delivery now uses `"type": "http"` hooks — Claude Code POSTs directly to `localhost:27182/hook`. No jq dependency, no shell overhead, no field re-mapping.
- **8 new hook events**: TeammateIdle, TaskCompleted, TaskCreated, CwdChanged, PreCompact, PostCompact, PostToolUseFailure, StopFailure
- **Team badge**: Shows active teammate count on session pills when using agent teams
- **Context compaction display**: Shows "Compacting" status with purple animation during context window compaction
- **CwdChanged tracking**: Sessions update correctly when working directory changes mid-session

### Removed
- Shell-based hook delivery (`notch-hook.sh` no longer used)
- `PermissionRequest/ExitPlanMode` hook entry (leftover from removed TTS feature)

## [1.12] - 2025-01-28

### Performance
- **Mouse tracking**: Reduced from 10fps to 5fps with 5px movement threshold (skips tiny movements)
- **Animation frequencies**: Activity indicator floor reduced from 12Hz to 8Hz, color shimmer from 6Hz to 4Hz
- **Memory pressure handling**: Added `DispatchSource` monitoring for aggressive cleanup on low memory
- **FSEvents file watcher**: Ralph loop detection now uses real-time file system events instead of polling (95% I/O reduction)
- **Session state cleanup**: Removed unused fields (`recentTools`, token tracking, `TokenStats` struct)

### Changed
- Ralph loop fallback scan interval increased from 30s to 60s (FSEvents handles most updates)
- Notch pills are fully click-through again (hover detection via global mouse monitor)
