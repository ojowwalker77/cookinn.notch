...
final class SetupManager: ObservableObject {
    ...
    private let httpHookUrl = "http://localhost:27182/hook"
    ...
    func getInstallCommand() -> String {
        ...
        for path in possiblePaths {
            let scriptPath = URL(fileURLWithPath: path)
                .appendingPathComponent("Contents/Resources/hooks/install.sh").path
    ...
    private func findHooksInBundle() -> URL? {
        ...
        let locations = [
            URL(fileURLWithPath: "/Applications/cookinn.notch.app/Contents/Resources"),
            FileManager.default.homeDirectoryForCurrentUser
                .appendingPathComponent("Applications/cookinn.notch.app/Contents/Resources")
        ]

        for location in locations {
            let hookScript = location.appendingPathComponent("notch-hook.sh")
            if FileManager.default.fileExists(atPath: hookScript.path) {
                return location
            }
        }
        ...
        for hookType in hookTypes {
            ...
            let alreadyExists = existingHooks.contains { entry in
                if let hookObjects = entry["hooks"] as? [[String: Any]] {
                    return hookObjects.contains { ($0["url"] as? String) == httpHookUrl }
                }
                return false
            }
            ...
            if let hookArray = value as? [[String: Any]] {
                for hook in hookArray {
                    if let hookObjects = hook["hooks"] as? [[String: Any]] {
                        if hookObjects.contains(where: { ($0["url"] as? String) == httpHookUrl }) {
                            return true
                        }
                    }
                }
