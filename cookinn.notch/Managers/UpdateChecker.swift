...
class UpdateChecker: ObservableObject {
    ...
    func performBrewUpgrade() {
        ...
        Task.detached { [weak self] in
            do {
                // Step 1: brew update (direct execution, no shell)
                let updateProcess = Process()
                updateProcess.executableURL = URL(fileURLWithPath: brewExecutable)
                ...
                guard updateProcess.terminationStatus == 0 else {
                ...
                // Step 2: brew upgrade (direct execution, no shell)
                let upgradeProcess = Process()
                upgradeProcess.executableURL = URL(fileURLWithPath: brewExecutable)
                upgradeProcess.arguments = ["upgrade", "cookinn-notch"]
                upgradeProcess.standardOutput = FileHandle.nullDevice
                upgradeProcess.standardError = FileHandle.nullDevice

                try upgradeProcess.run()
                upgradeProcess.waitUntilExit()

                await MainActor.run { [weak self] in
                    if upgradeProcess.terminationStatus == 0 {
    ...
    func restartApp() {
        ...
        // Schedule relaunch after brief delay using native APIs (no shell)
