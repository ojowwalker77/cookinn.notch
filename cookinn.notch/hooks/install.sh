...
update_settings() {
    ...
    local http_url="http://localhost:27182/hook"
    local hook_entry="{\"hooks\": [{\"type\": \"http\", \"url\": \"$http_url\"}]}"
    ...
    for hook_type in "${hook_types[@]}"; do
        ...
        local already_exists=$(echo "$existing" | jq --arg url "$http_url" '
            any(.[]; .hooks | any(if type == "object" then .url == $url else false end))
        ')
