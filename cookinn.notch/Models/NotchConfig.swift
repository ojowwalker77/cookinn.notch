...
//  cookinn.notch
//
//  Semantic Visual Language Configuration
...
struct ResolvedToolInfo: Equatable {
    let displayName: String
    let color: String
    let pattern: String
    let intensity: Int
...
final class ConfigManager: @unchecked Sendable {
    ...
    func resolveToolInfo(for name: String) -> ResolvedToolInfo {
        ...
        let resolvedPattern = toolConfig.pattern ?? categoryConfig?.pattern ?? "breathe"
    ...
    func patternConfig(for name: String) -> PatternConfig? {
        let pattern = config?.patterns[name]
        return pattern
    }
